...
def extract_path_rect(values, x_grid, y_grid, fraction=1.0, true_edge_lengths=None):
    """Extract values on a path through a quadrant of the Brillouin zone.
    ...
    >>> )
    """
    assert x_grid.shape[0] == y_grid.shape[0]
    assert x_grid.shape[1] == y_grid.shape[1]
    ny, nx = x_grid.shape
    dx = x_grid[0, 1] - x_grid[0, 0]
    dy = y_grid[-2, 0] - y_grid[-1, 0]
    if true_edge_lengths is None:
        ...
        true_edge_length_x, true_edge_length_y = true_edge_lengths
    if fraction == 1.0:
        path = np.zeros((nx + ny - 1, 4), dtype=values.dtype) 
        # bottom row
        s = np.full(shape=nx, fill_value=ny - 1), np.arange(0, nx, 1, dtype=int)
        grids = (x_grid, y_grid, values)
        for i, grid in zip(range(3), grids):
            path[:nx, i] = grid[s]
        # rightmost column
        s = np.arange(ny - 2, -1, -1, dtype=int), np.full(ny - 1, fill_value=nx - 1)
        for i, grid in zip(range(3), grids):
            path[nx : nx + ny - 1, i] = grid[s]
        # Coordinate along path
        path[:nx, 3] = path[:nx, 0]
        path[nx : nx + ny - 1, 3] = path[nx : nx + ny - 1, 1] + true_edge_length_x
    else:
        rows, cols = np.where(
            (x_grid <= fraction * true_edge_length_x)
            & (y_grid <= fraction * true_edge_length_y)
