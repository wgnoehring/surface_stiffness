...
def main():
    ...
    parser.add_argument("num_free_planes", type=int, help="Number of free planes")
    args = parser.parse_args()
    sparse_hessian = load_npz(args.ath_to_hessian.as_posix())
    inverse, dense = calculate_surface_greens_functions(
        sparse_hessian, num_atoms_per_plane, num_free_planes
    )
    path_inverse = Path(
...
def calculate_surface_greens_functions(
    sparse_hessian, num_atoms_per_plane, num_free_planes
):
